...
Keepers monitor the blockchain to keep track of which vaults have incurred bad debt. Once the total amount of bad debt exceeds the BDL, keepers can trigger a bad debt auction by spending the corresponding vaults and creating a bad debt auction coin.

(Any XCH that was still locked-up in the vaults gets transferred to the bad debt auction coin.) Any XCH that is still locked-up in the vaults get transferred to the Protocol Treasury. Once the amount of XCH that has accumulated in the Treasury exceeds the **XCH Treasury Maximum Balance**, an **XCH Surplus Auction** can be triggered to discharge the XCH from the system.
