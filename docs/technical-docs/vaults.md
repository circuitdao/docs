...
Vaults are smart coins in which XCH can be locked up as collateral. Vaults are user-specific, and can be used to borrow BYC up the the limit set by the liquidation ratio.

If the value drops below the liquidation threshold, keepers can trigger a vault liquidation.
