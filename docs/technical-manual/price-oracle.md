...
The mutate operation retains only the most recent matured price info as well as all non-matured price infos. All other matured price infos are discarded.
...
The mutate operation appends exactly one new price info to the ```PRICE_INFOS``` list. It may discard zero or more price infos from the beginning or the ```PRICE_INFOS``` list.
