...
(3) ```STATUTE_CUSTOM_CONDITIONS```:

(4) ```STATUTE_ORACLE_M_OF_N```:

(5) ```STATUTE_ORACLE_PRICE_UPDATE_DELAY```:

(6) ```STATUTE_ORACLE_PRICE_UPDATE_RATIO_BPS```:

(7) ```STATUTE_PRICE_DELAY```:

(8) ```STATUTE_VAULT_MINIMUM_DEBT```:

(9) ```STATUTE_VAULT_LIQUIDATION_RATIO_PCT```:

(10) ```STATUTE_VAULT_LIQUIDATION_PENALTY_BPS```:

(11) ```STATUTE_VAULT_INITIATOR_INCENTIVE_FLAT```:

(12) ```STATUTE_VAULT_INITIATOR_INCENTIVE_BPS```:

(13) ```STATUTE_VAULT_AUCTION_TTL```:

(14) ```STATUTE_VAULT_AUCTION_STARTING_PRICE_FACTOR_BPS```:

(15) ```STATUTE_VAULT_AUCTION_PRICE_TTL```:

(16) ```STATUTE_VAULT_AUCTION_PRICE_DECREASE_BPS```:

(17) ```STATUTE_VAULT_AUCTION_MINIMUM_PRICE_FACTOR_BPS```:

(18) ```STATUTE_VAULT_AUCTION_MINIMUM_BID_FLAT```:

(19) ```STATUTE_VAULT_AUCTION_MINIMUM_BID_BPS```:

(20) ```STATUTE_TREASURY_MINIMUM```:

(21) ```STATUTE_TREASURY_MAXIMUM```:

(22) ```STATUTE_TREASURY_MINIMUM_DELTA```:

(23) ```STATUTE_TREASURY_REBALANCE_RATIO_PCT```:

(24) ```STATUTE_AUCTIONS_MINIMUM_PRICE_INCREASE_BPS```:

(25) ```STATUTE_RECHARGE_AUCTION_TTL```:

(26) ```STATUTE_RECHARGE_AUCTION_MINIMUM_CRT_PRICE```:

(27) ```STATUTE_RECHARGE_AUCTION_BID_TTL```:

(28) ```STATUTE_RECHARGE_AUCTION_MINIMUM_BID```:

(29) ```STATUTE_RECHARGE_AUCTION_MAXIMUM_BID```:

(30) ```STATUTE_SURPLUS_AUCTION_LOT```:

(31) ```STATUTE_SURPLUS_AUCTION_BID_TTL```:

(32) ```STATUTE_ANNOUNCER_REWARDS_INTERVAL_PRICE_UPDATES```:

(33) ```STATUTE_ANNOUNCER_REWARDS_PER_INTERVAL```:

(34) ```STATUTE_ANNOUNCER_MINIMUM_DEPOSIT_MOJOS```:

(35) ```STATUTE_ANNOUNCER_MAXIMUM_VALUE_TTL```:

(36) ```STATUTE_ANNOUNCER_PENALTY_INTERVAL_MINUTES```:

(37) ```STATUTE_ANNOUNCER_PENALTY_PER_INTERVAL_BPS```:

(38) ```STATUTE_ANNOUNCER_DISAPPROVAL_MAXIMUM_PENALTY_BPS```:

(39) ```STATUTE_ANNOUNCER_DISAPPROVAL_COOLDOWN_INTERVAL```:

(40) ```STATUTE_GOVERNANCE_BILL_PROPOSAL_FEE_MOJOS```:

(41) ```STATUTE_GOVERNANCE_IMPLEMENTATION_INTERVAL```:

(42) ```STATUTE_GOVERNANCE_COOLDOWN_INTERVAL```:

(43) ```STATUTE_BLOCK_ISSUANCE```:
