...
* Surplus Auction start: keeper can select any subset of Treasury coins to prove that Treasury balance is greater than **[Treasury Maximum](../../user-guide/treasury)** plus **[Surplus Auction Lot Amount](../../user-guide/surplus-auction)**. For each selected Treasury coin, must specify a withdraw amount such that the sum of all withdraw amounts equals the Surplus Auction Lot Amount.

**Whole Treasury Ring:**

* Recharge auction start: must spent all Treasury coins to verify that Treasury balance is below **[Treasury Minimum](../../user-guide/treasury)**. No funds are deposited to or withdrawn from Treasury.
